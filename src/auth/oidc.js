...
export function createOidc(config) {
  ...
  return {
    async begin() {
      ...
      const url = oidc.buildAuthorizationUrl(provider, { redirect_uri: config.callback, scope: "openid profile",
    ...
    async finish(url, attempt) {
      const provider = await configured();
      const tokens = await oidc.authorizationCodeGrant(provider, url, { pkceCodeVerifier: attempt.verifier,
