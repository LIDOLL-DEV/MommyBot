...
const identities=new IdentityStore(":memory:"),store=new TouhouStore(":memory:",loadCatalog());
const user=identities.gameAccount({issuer:"https://auth.example",subject:"fixture",username:"Doll"}).player_id,guild="public";
...
try{
  ...
  const game=new TouhouWebGame(handlers.webState,publicGameAccess(identities,{list:async()=>{throw Error("Unexpected Discord request");},require:async()=>{throw Error("Private world");}}));
