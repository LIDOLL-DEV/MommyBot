...
import { ReportClient, ReportError, reportConfig } from "../src/reports/client.js";
...
try {
  ...
  console.error(`FAIL: ${error instanceof ReportError ? error.code : "configuration_unavailable"}${error instanceof ReportError && error.status ? ` (HTTP ${error.status})` : ""}. Check the protected report settings and tracker admin access.`);
  process.exitCode = 1;
